...
Galv.MAPT.setLocation = function(mapid,name,image,mx,my,tmid,tx,ty,desc,frames,shipX,shipY) {
	if (!Galv.MAPT.hasMap(mapid)) return;
	$gameSystem._travelMaps.maps[mapid].locations[name] = {name:name, image:image, mapXY:[mx,my], transfer:[tmid,tx,ty], shipTarget:[shipX,shipY], enabled:true, desc: desc,frames:frames || Galv.MAPT.defaultFrames};
};
...
Galv.MAPT.initLocation = function(name) {
	// Enable previous location if it exists
	if ($gameSystem._travelMaps.current) {
		for (var id in $gameSystem._travelMaps.maps) {
			if ($gameSystem._travelMaps.maps[id].locations[$gameSystem._travelMaps.current]) {
				$gameSystem._travelMaps.maps[id].locations[$gameSystem._travelMaps.current].enabled = true;
			}
		}
	}
	$gameSystem._travelMaps.current = name;
	// Disable new current location
	for (var id in $gameSystem._travelMaps.maps) {
		if ($gameSystem._travelMaps.maps[id].locations[name]) {
			$gameSystem._travelMaps.maps[id].locations[name].enabled = false;
		}
	}
};
...
Scene_MapTravel.prototype.doTransfer = function() {
	...
	var shipVars = this.data().locations[Galv.MAPT.active].shipTarget || [];
	$gameVariables.setValue(91, shipVars[0]);
	$gameVariables.setValue(92, shipVars[1]);
	// Enable old location and disable new one
	if ($gameSystem._travelMaps.current) {
		this.data().locations[$gameSystem._travelMaps.current].enabled = true;
	}
	this.data().locations[Galv.MAPT.active].enabled = false;
	$gameSystem._travelMaps.current = Galv.MAPT.active;
...
Scene_MapTravel.prototype.updateRefresh = function() {
	if (this._needRefresh) {
		// Center the map sprite
		var mapWidth = this.mapSprite.bitmap.width;
		var mapHeight = this.mapSprite.bitmap.height;
		this.mapSprite.x = (Graphics.boxWidth - mapWidth) / 2;
		this.mapSprite.y = (Graphics.boxHeight - mapHeight) / 2;
